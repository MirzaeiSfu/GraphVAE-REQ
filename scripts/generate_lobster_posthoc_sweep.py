#!/usr/bin/env python3
"""Generate the controlled 14-config Lobster post-training sweep."""
...
def main() -> None:
    ...
    for kia_weights in (False, True):
        ...
        for weight_name, node, edge, motif_weight in WEIGHTS:
            ...
            for rule_mode in rule_modes:
                ...
                motif["use_syntactic_literal_rules"] = rule_mode == "both"
                ...
                stem = f"lobster_{base_mode}_{weight_name}_{rule_mode}"
    ...
    for wave_index, start in enumerate(range(0, len(configs), len(SLOTS)), 1):
        ...
        for slot_index, path in enumerate(wave_configs):
            host, gpu = SLOTS[slot_index]
