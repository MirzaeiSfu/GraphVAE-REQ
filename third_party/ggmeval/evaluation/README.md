...
Local note: in this repo we keep only the GNN-based evaluation subset from upstream.
The structure-only MMD code and duplicate ORCA sources are not vendored here because
their counterparts already exist in our local `stat_rnn.py` and `eval/` stack.
...
```
In this trimmed vendor copy, the main entry points are:

- `evaluation.evaluator.Evaluator` for the full GNN-based metric bundle
- `evaluation.gin_evaluation` for the individual GIN-based metrics
- `../RuleEval.py` for the original `SaveSamples(...)` attributed-graph pickle outputs
- `../eval_all_in_dir_2023.py` for Kia-style generated-vs-reference directory evaluation
- `../eval_all_in_dir_5050.py` for 50/50 ideal-reference evaluation
